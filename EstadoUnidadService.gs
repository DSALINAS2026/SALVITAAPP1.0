...
function EU7_ss_(){
  ...
  // 2) Si existe propiedad guardada// ===================== ESTADO UNIDAD SERVICE (SAFE PATCH) =====================
// Este archivo NO declara SPREADSHEET_ID para evitar "already been declared".
// Usa nombres únicos EU7_* para no chocar con otros módulos.
...
// ===== Historial inline para Reprogramar (últimos N de PreventivosUnidadHis) =====
function EU_getReprogHistory(token, interno, idHP, limit){
  try{
    // token opcional (no bloquea historial)
    interno = String(interno||"").trim();
    idHP = String(idHP||"").trim();

    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const sh = ss.getSheets().find(s => /preventiv.*unidad.*his/i.test(s.getName()));
    ...
    const idx = (name) => head.indexOf(String(name).toLowerCase());
    const cInterno = idx("interno");
    const cIdHP    = idx("idhp");
    const cAccion  = idx("accion");
    const cAntes   = idx("antes");
    const cDespues = idx("despues");
    const cMotivo  = idx("motivo");
    const cUsuario = idx("usuario");
    const cTs      = idx("timestamp");

    const wantInterno = interno;
    const wantId = idHP.toLowerCase();

    const items = [];
    for (let i = rows.length - 1; i >= 0; i--){
      const r = rows[i];
      const rInterno = (cInterno>=0 ? String(r[cInterno]||"").trim() : "");
      const rId = (cIdHP>=0 ? String(r[cIdHP]||"").trim().toLowerCase() : "");
      if (wantInterno && rInterno !== wantInterno) continue;

      // Match flexible: exact, or contained (por si vienen espacios/raro)
      if (wantId){
        if (!rId) continue;
        if (rId !== wantId && !rId.includes(wantId) && !wantId.includes(rId)) continue;
      }

      items.push({
        ts: cTs>=0 ? r[cTs] : "",
        accion: cAccion>=0 ? r[cAccion] : "",
        antes: cAntes>=0 ? r[cAntes] : "",
        despues: cDespues>=0 ? r[cDespues] : "",
        motivo: cMotivo>=0 ? r[cMotivo] : "",
        usuario: cUsuario>=0 ? r[cUsuario] : ""
      });

      if (items.length >= (Number(limit)||5)) break;
    }

    return { ok:true, items:items };
  }catch(e){
    // Historial nunca debe romper el UI
    return { ok:true, items:[], warn:String(e) };
  }
}

// Compacta JSON del his (Antes/Despues) para mostrarlo en una línea
function EU7_compactHis_(v){
  try{
    if (v == null) return "";
    if (typeof v === "string"){
      const s = v.trim();
      ...
      if (s[0] === "{" || s[0] === "["){
        ...
        return EU7_compactObj_(o);
      }
      return s;
    }
    if (typeof v === "object"){
      return EU7_compactObj_(v);
    }
    return String(v);
  }catch(_e){
    return String(v||"");
...
function EU7_compactObj_(o){
  if (!o || typeof o !== "object") return String(o||"");
  const parts = [];
  if (o.UltimoKm != null && o.UltimoKm !== "") parts.push("UltKm " + o.UltimoKm);
  if (o.UltimaFecha) parts.push("UltF " + EU7_fmtDate_(o.UltimaFecha));
  if (o.ProximoKm != null && o.ProximoKm !== "") parts.push("ProxKm " + o.ProximoKm);
  if (o.ProximaFecha) parts.push("ProxF " + EU7_fmtDate_(o.ProximaFecha));
  return parts.join(" · ");
}
