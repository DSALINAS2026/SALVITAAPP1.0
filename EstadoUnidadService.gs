...
function reprogramarPreventivoUnidadV8(token, interno, idHP, nuevoUltimo, motivo){
  // V8: misma lógica que V7 pero con retorno "a prueba de fallos":
  // si ya se guardó en PreventivosUnidad (y/o His), jamás devuelve ok:false por errores posteriores.
  let didSave = false;
  let didHis = false;
  let rowIdx = -1;
  let h = null;
  ...
  try{
    ...
    const lr = shPU.getLastRow();
    const lc = shPU.getLastColumn();
    if (lr < 2) return {ok:false, msg:'PreventivosUnidad vacío'};

    h  = shPU.getRange(1,1,1,lc).getValues()[0];
    ix = EU7_idx_(h);
    const v  = shPU.getRange(2,1,lr-1,lc).getValues();

    // Buscar fila (Interno + IdHP)
    const iInterno = EU7_norm_('Interno');
    const iIdHP    = EU7_norm_('IdHP');
    ...
    if (control === 'dia'){
      const s = (nuevoUltimo ?? '').toString().trim(); // YYYY-MM-DD
      if (!/^\d{4}-\d{2}-\d{2}$/.test(s)) return {ok:false, msg:'Fecha inválida'};
      const dt = new Date(s + 'T00:00:00');
      if (isNaN(dt.getTime())) return {ok:false, msg:'Fecha inválida'};

      EU7_set_(row, ix, 'UltimaFecha', dt);
      if (cadaDias > 0){
        const p = new Date(dt.getTime());
        p.setDate(p.getDate() + cadaDias);
        EU7_set_(row, ix, 'ProximaFecha', p);
      }
    } else {
    ...
    // Guardar PreventivosUnidad
    shPU.getRange(rowIdx+2, 1, 1, h.length).setValues([row]);
    didSave = true;

    // Historial robusto
    if (shHis){
      shHisName = shHis.getName();
      const hv = shHis.getRange(1,1,1,shHis.getLastColumn()).getValues()[0];
      const hx = EU7_idx_(hv);
      const out = new Array(hv.length).fill('');
      ...
      setH('Interno', interno);
      setH('IdHP', idHP);
      setH('NombreHP', EU7_get_(row, ix, 'NombreHP','Preventivo','Nombre'));
      setH('Accion', 'REPROG');
      setH('Antes', JSON.stringify(antes || {}));
      setH('Despues', ''); // se completa abajo si se puede
      setH('Motivo', motivo);
      setH('Usuario', (typeof getSessionUser === 'function') ? (getSessionUser(token)||'') : '');
      setH('Timestamp', new Date());
    ...
    // Intentar leer "después" para devolverlo (pero NO fallar si algo sale mal)
    try{
      SpreadsheetApp.flush();
      const afterRow = shPU.getRange(rowIdx+2, 1, 1, h.length).getValues()[0];
      despues = {
    ...
    return {ok:true, row: rowIdx+2, antes: antes || null, despues: despues, hisSheet: shHisName};

  }catch(e){
    // Si ya guardamos, jamás devolvemos ok:false (el cambio ya impactó)
    if (didSave || didHis){
      return {ok:true, warn:true, row: (rowIdx>=0 ? rowIdx+2 : ''), msg:'Reprogramado (con advertencia)', error:String(e)};
    }
    return {ok:false, msg:'No se pudo reprogramar', error:String(e)};
  }
}
